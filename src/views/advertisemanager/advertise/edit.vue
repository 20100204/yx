			...
			<Card>
				<Form :label-width="80" :model="ad" :rules="ruleValidate" ref="ad">
					<FormItem label="广告标题" prop="title" :error="articleError">
				   	...
				   	<FormItem label="广告位置" prop="admin_id">
					...
					<FormItem label="链接id" prop="value" v-if="ad.type != 'pic' ">
						<InputNumber v-model="ad.value" placeholder="请输入广告链接id" icon="android-list" style="width:200px;" :min="0"></InputNumber >
					...
					<!--<FormItem label="链接地址" prop="url">-->
						<!--<Input type="url"   v-model="ad.url" placeholder="请输入广告链接地址" icon="android-list" style="width:600px;" />-->
...
<script>
	...
	export default {
		...
		beforeCreate() {
			...
			this.$http.get('advertise/edit&id='+id).then((res) => {
				//console.log(res);
				res.data.ad.rank = parseInt(res.data.ad.rank);
				this.ad = res.data.ad;
				this.pos = res.data.location;
				this.types = res.data.types;
				this.isCity = Boolean(res.data.isCity);
				if(this.isCity){
				    this.ad.area=res.data.city;
				}else{
			...
			handlePublish(name) {
				...
				this.$refs[name].validate((valid) => {
					if(valid) {
						...
						this.$http.post('advertise/editsave', {ad:this.ad}).then((res) => {
							if(res.data.rs == "true") {
