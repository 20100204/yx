	...
	export default {
		...
		methods: {
			...
			handleSuccess(evnet, file, rs) {
				var ob = file;
				console.log(file);
				ob.url =  file.response;
				//this.$store.state.ycypbaseurl +
				this.uploadpics.push(ob);
				//var len = this.uploadpics.length;
				//this.uploadpics[len-1].url=
				this.$Notice.success({
					title: '文件上传成功',
			...
			handleRemove(file) {
				...
				 this.uploadpics.splice(this.uploadpics.indexOf(file),1);
