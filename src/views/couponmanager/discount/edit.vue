        ...
        <Row>
            ...
            <Card>
                <Form
                    ...
                    <FormItem label="最小可用订单金额" :error="articleError" prop="min_amount">
                        <InputNumber v-model="couponcash.min_amount" placeholder="最小可用订单金额" :max="1000000" :min="0.01" :step="0.01"></InputNumber>
                    </FormItem>
                    <FormItem label="优惠券金额" :error="articleError" prop="discount_amount">
...
<script>
    export default {
        ...
        mounted(){
            this.$http.get('coupondiscount/edit&id='+this.$route.params.id).then((res)=>{
                this.isCity = Boolean(res.data.isCity);
                res.data.rs.discount_amount = parseFloat(res.data.rs.discount_amount);
                res.data.rs.min_amount = parseFloat(res.data.rs.min_amount);
            ...
            handlePublish(name) {
                this.$refs.coupon_cash.validate((valid) => {
                    if(valid) {
                        ...
                        this.$http.post('coupondiscount/edit', {
