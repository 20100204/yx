	...
	export default {
		...
		data() {
			...
			const valideOldPassword = (rule, value, callback) => {
				if(value !== this.oldpass) {
					callback(new Error('与老密码不一致'));
				} else {
					callback();
				}
			};
		...
		methods: {
			...
			saveEditPass() {
				this.$refs['editPasswordForm'].validate((valid) => {
					if(valid) {
						...
						this.$http.post('citycmp/repasswd',{id:this.uid,passwd:this.editPasswordForm.newPass}).then((res) => {
							if(res.data.rs == 'true') {
								this.$Notice.success({
									title: '修改成功',
									desc: '修改成功',
			...
			init() {
				this.$http.get('citycmp/userinfo').then((res) => {
					console.log(res);
					this.uid = res.data.userId;
					this.oldpass = res.data.passwd;
				}).catch((err) => {
					console.log(err);
				});
