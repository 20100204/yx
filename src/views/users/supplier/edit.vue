        ...
        <Card>
            <Form :label-width="120" :model="supplier" :rules="ruleValidate" ref="supplier">
                <FormItem label="登录账户" prop="username" :error="articleError">
                    <Input  v-model="supplier.username" placeholder="请输入登录账户名" :maxlength="20"  :autofocus="true" style="width:200px;" :disabled="true" />
                </FormItem>
                ...
                <FormItem label="详细地址" prop="address" :error="articleError">
                    <Input  v-model="supplier.address" placeholder="请输入详细地址" :maxlength="20"    style="width:500px;"/>
                </FormItem>
...
<script>
    ...
    export default {
            ...
            return {
                ...
                ruleValidate:{
                    ...
                    supplier_name:[
                        ...
                        {min:2,max:30,message:"长度为2到30之间",trigger:"blur"}
                        ],
                    contact:[
        ...
        methods:{
            init(){
                this.$http.get('supplier/edit&id='+this.$route.params.id).then((res)=>{
                      this.isCity = Boolean(res.data.isCity);
                      this.supplier = res.data.rs;
                      this.newcitylist = res.data.citys;
                      this.cityCompany = [res.data.rs.province,res.data.rs.city,res.data.rs.county];
                }).catch((err)=>{
                    console.log(err);
                });
            },
            ...
            handlePublish(name){
                this.$refs[name].validate((valid) => {
                    ...
                    if (valid) {
                        this.$http.post('supplier/edit',{supplier:this.supplier}).then((res)=>{
                            if(res.data.rs==='true'){
