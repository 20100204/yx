			...
			<Card>
				<Form :label-width="120" :model="user" :rules="ruleValidate" ref="user">
					<!--<FormItem label="登录账户" prop="username" :error="articleError">
						<Input :disabled="disabled" v-model="user.username" placeholder="请输入登录账户名" icon="android-list" style="width:600px;" />
					</FormItem> -->
					<!--<FormItem label="登录密码" prop="password" :error="articleError">
						<Input v-model="user.password" type="password" placeholder="请输入登录密码" icon="android-list" style="width:600px;" />
					</FormItem>-->
					...
					<FormItem label="mobile" prop="mobile" :error="articleError">
						<Input v-model="user.mobile" placeholder="请填mobile" style="width: 200px" />
					</FormItem>
				<!--	<FormItem label="email" prop="email" :error="articleError">
					...
					<FormItem label="店铺或代理名称" prop="shop_name" :error="articleError">
						<Input type="email" v-model="user.shop_name" placeholder="请填店铺或代理名称名称" style="width: 200px" />
					...
					<FormItem label="所属分公司" prop="admin_id" v-if= "roleId!=2">
						<Select v-model="user.admin_id" style="width:200px" placeholder="请选所属分公司" :disabled="roleId==2?true:false">
							<Option v-for="item in newcitylist" :value="item.id">{{ item.company_name }}</Option>
						</Select>
					</FormItem>
					...
					<FormItem label="地址详情" prop="address">
						<Input v-model="user.address" placeholder="请填地址详情" style="width: 200px" />
					</FormItem>
					...
					<FormItem>
						...
						<!--<Button type="ghost" @click="handleReset('user')" style="margin-left: 8px">
							 	 重置
						</Button>-->
					</FormItem>
				</Form>
	...
	export default {
		...
		methods: {
			...
			handlePublish(name) {
				...
				this.$refs[name].validate((valid) => {
					...
					if(valid) {
						this.$http.post('shoper/editsave', {
							user: this.user
						}).then((res) => {
							if(res.data.rs == "true") {
								this.$router.push({
		...
		mounted() {
			...
			var id =this.$route.params.id;
			  this.$http.get('shoper/edit&id='+id).then((res) => {
			   	 this.user = res.data.rs;
			   	 this.user.profit_rate= parseFloat( this.user.profit_rate);
			  	 this.cityList = res.data.fenggongsi;
